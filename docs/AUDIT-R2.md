...
| P-3: arbitrary approved-host image CIDs | Full last-good catalogue membership is checked before cache hits, conditional responses and upstream fetching. Cold reads coalesce; stale/excluded logos remain displayable. A 25-second shared deadline bounds relay work; current image URLs use `v=2` and must revalidate. Updated clients bypass prior immutable CDN entries; old cached URLs are not claimed to be purged. Catalogue membership constrains upstream work; it does not make known-catalogue traffic immune to denial of service. |
...
The first candidate failed its native Node ESM import before alias promotion. Explicit JSON import attributes and an emitted-module test corrected it. The final application revision is `ed67a05f7b676108a98dcaf29460d2fdc60fecc6`: 258 tests in 20 files, typecheck/build, desktop/mobile inspection, eight deployed asset comparisons and ten decoded catalogue logos passed. See `evidence/r2/validation.json`. The image relay rejects an unlisted approved-host CID on the deployed revision. Source-health and image authorization each received internal specialist review; the final native ESM fix received the regression and deployed execution checks.
