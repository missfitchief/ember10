...
6. Explicit go/no-go decisions for read-only public publication and for any future funded pilot. These are separate decisions.

Preserve EMBER10, ten equal purchase budgets, the 80/10/10 allocation, existing OPS/DEV remainder rules and immutable historical funded records. Do not mark undocumented evidence as passing eligibility. The current public market ranking is not a funded basket.

Use an isolated disposable database and fixtures. Do not supply funds, enable signing/broadcast, resume a production worker, migrate a production database or change a deployment. Read-only public/provider verification is within scope; credentials and live financial actions require a separate owner request. Report unresolved F-1 through F-14 and the dependency findings rather than interpreting synthetic test success as financial readiness.
