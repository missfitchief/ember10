...
Read timeout 8 seconds per attempt, 2 attempts, 1 second backoff, no redirects, 64 MB streaming source cap, and 50,000 row schema cap. Only the byte cap is shared with the evidence collector through `packages/shared/catalogue-limits.json`; the collector performs one 20-second request, without runtime retries or a row cap. At roughly 2.6 KB per row, the byte cap is reached near 24,600 rows, before the 50,000-row cap. At 80% of either runtime capacity, a structured `catalogue_capacity` warning and public coverage note request operator review; these are not an externally delivered alert. Breaches remain failures, never truncated catalogues; stale last-good data remains explicitly stale. Concurrent reads share one in-flight request and a 45 second instance cache. Unavailable initial reads return no markets, and the hosted basket does not label an unavailable or partial projection complete. Cold starts do not import a seed fixture.
