...
export function walletResult(input: PublicWalletRewards|PublicRecord, expectedAddress?: string): WalletResult {
  ...
  if (expectedAddress && v.address !== expectedAddress) throw Error('The reward response did not match the requested address.');
  if (typeof v.address !== 'string' || !v.address || !Array.isArray(v.entitlements) || !Array.isArray(v.deliveries)
    || v.status != null && !['available','unavailable'].includes(String(v.status))) throw Error('The service returned an unreadable reward record. No balance has been inferred.');
...
export function ledgerConnection(v: PublicRecord): { availability: 'available' | 'unavailable'; ledger: Ledger | null; message: string } {
  ...
  return { availability: 'available', ledger, message: 'The public reward ledger is connected.' };
}
