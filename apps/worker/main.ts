...
if(c.MODE==='live'){jupiter=new JupiterClient(c.JUPITER_API_KEY!);const signer=new RemoteSigner(new PublicKey(c.TREASURY!),c.SIGNER_URL!,c.SIGNER_TOKEN??'');const connection=new Connection(c.RPC_URL,'finalized');chain=new SolanaChain(c,signer,undefined,async i=>{const a=await loadApproval(c);return safeJupiterBuild(connection,jupiter!,signer.publicKey,i,a.approvedPrograms,await admitFundedMint(db,i,a),a.policy);});await chain.verifyCluster();}
function currentSelection(a:Approval){const key=hash(a);if(!selection||key!==selectionKey){ensure(chain instanceof SolanaChain&&jupiter,'live evidence adapters required');selection=new AutomaticSelectionService(a.policy,a.ourMint,createUniverseProvider(chain.connection,ember,jupiter,a,c.CENSUS_COMPLETE_CONTRACT));selectionKey=key;}return selection;}
