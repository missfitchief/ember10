...
for(let pass=0;pass<2;pass++){
 ...
 const times=rows.map(r=>r.ms).sort((a,b)=>a-b),mid=Math.floor(times.length/2);
 report.passes.push({pass,medianMs:times.length%2?times[mid]:(times[mid-1]+times[mid])/2,rows});
}
