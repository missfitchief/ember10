...
try{
 ...
 for(const market of data.markets){const response=await get('/api/token-image?source='+encodeURIComponent(market.imageUrl)+'&v=2'),bytes=Buffer.from(await response.arrayBuffer()),meta=await sharp(bytes).metadata();assert.equal(response.headers.get('content-type'),'image/webp');assert.equal(meta.format,'webp');assert(meta.width>0&&meta.width<=160&&meta.height>0&&meta.height<=160);report.logos.push({mint:market.mint,bytes:bytes.length,width:meta.width,height:meta.height,sha256:hash(bytes)});}
 report.passed=true;
